...
CLI. Sentinel controls which templates run through its Template Registry and the
...
class NucleiAdapter(ContainerEngineAdapter):
    ...
    async def prepare_target(self, request: EngineRunRequest) -> PreparedRun:
        ...
        # Template selection is controlled by Sentinel (Template Registry), never
        ...
        for url in request.seed_urls:
            args += ["-u", url]
        # Force egress through Sentinel's scope-enforcing proxy when provided.
        env: dict[str, str] = {}
