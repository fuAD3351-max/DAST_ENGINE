...
Commands:
    ...
    sentinel license check      validate the third-party inventory (CI gate)
    sentinel engine list        list engines with license/approval/usability
    sentinel engine verify      check version/digest pinning
    sentinel scan run           run a scan against an authorized target
...
app = typer.Typer(
    name="sentinel",
    help="Sentinel DAST - unified dynamic application security testing platform.",
...
def version() -> None:
    ...
    typer.echo(f"Sentinel DAST {__version__}")
...
def license_check(
    ...
    dependency.
    """
    from sentinel.governance.inventory import Inventory, check_inventory
