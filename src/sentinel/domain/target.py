...
:class:`AuthorizationRecord`. This is a product-level safety control: Sentinel
...
class AuthorizationMethod(StrEnum):
    ...
    DNS_TXT = "dns_txt"  # TXT record with a Sentinel verification token
    HTTP_FILE = "http_file"  # /.well-known/sentinel-verification.txt
...
class AuthorizationRecord(SentinelModel):
...
class RateLimits(SentinelModel):
