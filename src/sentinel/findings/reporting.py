...
from sentinel.domain import Severity, UnifiedFinding
from sentinel.scan.orchestrator import ScanReport
        ...
        results.append(
            {
                ...
                "partialFingerprints": {"sentinel/v1": f.fingerprint},
            }
        )
    sarif = {
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        ...
                        "informationUri": "https://sentinel.example/dast",
...
def to_markdown(report: ScanReport) -> str:
    ...
    lines: list[str] = []
    lines.append(f"# Sentinel DAST report - scan `{report.scan.id}`")
    lines.append("")
