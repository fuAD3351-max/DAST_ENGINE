...
------------------------
Burp Suite is proprietary, commercial software. Vantage does **not** bundle,
embed, containerize or redistribute Burp in any form. This adapter is
first-party Vantage code that speaks Burp's documented REST API to a Burp
...
* launches a scan constrained to Vantage's scope, polls to completion within a
  bounded budget, and maps Burp's issue model onto Vantage Observations;
* treats Burp's output as untrusted input, like every other engine;
* maps Burp confidence onto Vantage confidence conservatively - Burp's own
  "certain" becomes FIRM, never CONFIRMED, because CONFIRMED is reserved for
  Vantage's own validation engine.
...
# Burp confidence -> Vantage confidence. Never CONFIRMED (reserved for our
# validation engine).
_CONFIDENCE_MAP = {
...
class BurpAdapter(EngineAdapter):
    ...
    def metadata(self) -> EngineMetadata:
        return EngineMetadata(
            ...
            # Approved for Vantage to DISTRIBUTE (we ship only the API client);
