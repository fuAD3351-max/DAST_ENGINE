...
from vantage.domain.target import ScopeRule
...
def create_api(vantage_app: Any | None = None, *, tenant: str = "default") -> FastAPI:
    """Build the FastAPI app. If ``vantage_app`` is None it is built lazily."""
    from vantage.app import VantageApp

    api = FastAPI(title="Vantage DAST Control Plane", version=__version__)
    state: dict[str, Any] = {"app": vantage_app, "targets": {}, "reports": {}}
...
# Default ASGI app for `uvicorn vantage.api.app:app`.
app = create_api()
