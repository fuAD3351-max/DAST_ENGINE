...
class IntegrationType(StrEnum):
    NATIVE = "native"  # proprietary Vantage code, in-process
    ...
    # Vantage ships only an API client, never the third-party software (bring-your-own-license)


class NetworkMode(StrEnum):
    NONE = "none"  # no network at all (parsers, analyzers of stored artifacts)
    SCOPED_EGRESS = "scoped_egress"  # egress only via the Vantage scope-enforcing proxy


class ResourceLimits(VantageModel):
...
class EngineMetadata(VantageModel):
    id: Identifier
    name: str
    version: str
    vendor: str
...
class HealthStatus(VantageModel):
    state: HealthState
    detail: str = ""
    checked_at: datetime = Field(default_factory=utcnow)


class EngineRunRequest(VantageModel):
...
class EngineRunResult(VantageModel):
    run_id: str
    engine_id: Identifier
    engine_version: str
    status: RunStatus
